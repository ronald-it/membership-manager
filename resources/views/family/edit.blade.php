<x-layout>
  ...
  <div class="flex flex-col gap-y-6 lg:gap-y-16 p-4 sm:p-8 lg:p-16">
    <form class="flex justify-center" action="/family/{{$family->id}}" method="POST">
      @csrf
      @method('PUT')
      <div class="flex flex-col sm:flex-row sm:flex-wrap items-center gap-y-3 sm:gap-x-3 w-full max-w-md sm:max-w-3xl lg:max-w-7xl">
        <span class="uppercase font-medium self-start sm:w-full">family</span>
        <div class="w-full sm:w-auto">
          <div class="grow flex flex-col sm:flex-row sm:items-center">
            <label class="font-medium w-12 sm:w-fit sm:mr-3 capitalize" for="id">
              id
            </label>
            <input type="number" value="{{$family->id}}" id="id" name="id" readonly="true" class="border border-theme-purple rounded grow p-2 sm:w-16 sm:py-0 sm:h-8"/>
          </div>
        </div>
        ...
        <div class="grow w-full sm:w-auto">
          <div class="grow flex flex-col sm:flex-row sm:items-center">
            ...
            <input type="text" value="{{$family->address}}" id="address" name="address" class="border border-theme-purple rounded grow p-2 sm:w-20 sm:py-0 sm:h-8"/>
          </div>
        </div>
    ...
    <section class="flex justify-center">
      <div class="flex flex-col gap-y-3 w-full max-w-md sm:max-w-3xl lg:max-w-7xl">
        <div class="flex justify-between items-center">
          <span class="uppercase font-medium">family members</span>
          <form action="/family/{{$family->id}}" method="POST">
            @csrf
            @method('DELETE')
            <button type="submit" class="bg-theme-red text-white p-2 rounded uppercase flex items-center gap-x-2 text-nowrap">
    ...
    <form class="flex justify-center" action="/family/{{$family->id}}/family-member" method="POST">
      @csrf
      <div class="flex flex-col sm:flex-row sm:flex-wrap items-center gap-y-3 sm:gap-x-3 w-full max-w-md sm:max-w-3xl lg:max-w-7xl">
        <span class="uppercase font-medium self-start sm:w-full">add new family member</span>
        <div class="w-full sm:w-auto grow">
          <div class="grow flex flex-col sm:flex-row sm:items-center">
            <label class="font-medium w-24 sm:w-fit sm:mr-3 capitalize" for="name">
              name
            </label>
            <input type="text" class="border border-theme-purple rounded grow p-2 sm:w-16 sm:py-0 sm:h-8" id="name" name="name"/>
          </div>
        </div>
        <div class="w-full sm:w-auto">
          <div class="grow flex flex-col sm:flex-row sm:items-center">
            ...
            <input type="date" class="border border-theme-purple rounded grow p-2 sm:py-0 sm:h-8 sm:w-32" id="date_of_birth" name="date_of_birth"/>
          </div>
        </div>
